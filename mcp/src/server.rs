...
// ============================================================================
// Notes Parameter Types (SerenNotes service)
// ============================================================================
...
pub struct CreateNoteParams {
    ...
    pub content: String,
    /// Content format: markdown (default), org, or json
    #[serde(default)]
...
pub struct GetNoteParams {
    /// Note ID (UUID)
    pub note_id: Uuid,
    /// Output format: markdown (default), org, or json
    #[serde(default)]
    pub format: Option<String>,
}
...
/// Parameters for appending content to a note
#[derive(Debug, Deserialize, Serialize, JsonSchema)]
...
/// Parameters for adding tags to a note
#[derive(Debug, Deserialize, Serialize, JsonSchema)]
pub struct AddNoteTagsParams {
    /// Note ID (UUID)
    pub note_id: Uuid,
    /// Tags to add
    pub tags: Vec<String>,
...
/// Parameters for removing tags from a note
#[derive(Debug, Deserialize, Serialize, JsonSchema)]
pub struct RemoveNoteTagsParams {
    /// Note ID (UUID)
    pub note_id: Uuid,
    /// Tags to remove
    pub tags: Vec<String>,
...
impl SerenMcpServer {
    ...
    #[tool(
        description = "Create a new note. Store research, analysis, or any text content with optional hierarchical organization and tags.",
        annotations(
            read_only_hint = false,
            destructive_hint = false,
            open_world_hint = false
    ...
    async fn create_note(
        ...
        // Convert format string to NoteFormat enum
        let format = params.format.as_deref().and_then(|f| match f {
            "markdown" => Some(seren::NoteFormat::Markdown),
            "org" => Some(seren::NoteFormat::Org),
            "json" => Some(seren::NoteFormat::Json),
            _ => None,
        });

        let request = seren::CreateNoteRequest {
            title: params.title,
        ...
        Ok(CallToolResult::success(vec![json_content(&response.data)?]))
    }

    #[tool(
        description = "Get a note by ID. Returns the full note content including tags.",
        annotations(read_only_hint = true, open_world_hint = false)
    )]
    async fn get_note(
        &self,
        ...
        let response = api_client
            .get_note(&params.note_id, params.format.as_deref())
            .await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?
            .into_inner();
    ...
    #[tool(
        description = "Update an existing note. You can update the title, content, or archive/pin status.",
        annotations(
            read_only_hint = false,
            destructive_hint = false,
            open_world_hint = false
    ...
    async fn update_note(
        ...
        let request = seren::UpdateNoteRequest {
            title: params.title,
            content: params.content,
            format,
            parent_id: params.parent_id,
        ...
        let response = api_client
            .update_note(&params.note_id, &request)
            .await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?
            .into_inner();
    ...
    #[tool(
        description = "List notes with optional filtering by parent or tag. Returns a paginated list of notes.",
        annotations(read_only_hint = true, open_world_hint = false)
    )]
    async fn list_notes(
        &self,
    ...
    #[tool(
        description = "Search notes using full-text search. Returns notes matching the query ranked by relevance.",
        annotations(read_only_hint = true, open_world_hint = false)
    )]
    ...
    #[tool(
        description = "Append content to an existing note. Useful for incrementally adding research findings or logs.",
        annotations(
            read_only_hint = false,
            destructive_hint = false,
            open_world_hint = false
    ...
    async fn append_to_note(
        ...
        let request = seren::AppendToNoteRequest {
            content: params.content,
            format,
            idempotency_key: params.idempotency_key,
        };
        ...
        let response = api_client
            .append_to_note(&params.note_id, &request)
            .await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?
            .into_inner();
    ...
    #[tool(
        description = "Add tags to a note for organization and filtering.",
        annotations(
            read_only_hint = false,
            destructive_hint = false,
            open_world_hint = false
    ...
    async fn add_note_tags(
        &self,
        Parameters(params): Parameters<AddNoteTagsParams>,
        extensions: Extensions,
    ) -> Result<CallToolResult, McpError> {
        ensure_writes_allowed(&extensions)?;
        ...
        let request = seren::AddTagsRequest {
            tags: params.tags.clone(),
        };

        let response = api_client
            .add_tags(&params.note_id, &request)
            .await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?
            .into_inner();
    ...
    async fn remove_note_tags(
        &self,
        Parameters(params): Parameters<RemoveNoteTagsParams>,
        extensions: Extensions,
    ) -> Result<CallToolResult, McpError> {
        ensure_writes_allowed(&extensions)?;
        ...
        let request = seren::RemoveTagsRequest {
            tags: params.tags.clone(),
        };

        let response = api_client
            .remove_tags(&params.note_id, &request)
            .await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?
            .into_inner();
    ...
    async fn list_note_tags(&self, extensions: Extensions) -> Result<CallToolResult, McpError> {
        let api_client = self.api_client(&extensions)?;
