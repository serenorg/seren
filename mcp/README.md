...
| Object storage | List Seren Storage buckets, upload base64 payloads, create presigned uploads, list and download objects, delete by object ID or key, and manage agent grants and workspace snapshots |
...
- "List my Seren Storage buckets, upload this generated CSV to the customer-exports bucket, and delete the old export by key."
...
`seren_memory_delete_memories_by_source` permanently removes retained sources and every memory derived from them. It requires `source_external_id`, `source_uri`, or both; `project_id` can narrow the deletion. The tool is marked destructive and is blocked by MCP read-only mode. The dedicated managed-turn capture and capture-status operations, human review transitions, private-memory promotion, and knowledge administration remain outside the MCP tool catalog. Review and promotion requests bind the decision to an explicit memory revision, and organization-selected candidates also bind review to their signed capture domain.
...
- `get_seren_agent_model_credential_proposal` and `apply_seren_agent_model_credential_proposal` read and apply the current model credential proposal; supply `proposal_id` and the approved `setup_id` for `configure` with either API-key or ChatGPT-subscription authentication, and for `remove` with remaining grants. An already-applied retry or removal without remaining grants may omit `setup_id`.
