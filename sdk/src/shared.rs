...
///
/// The selectors are mutually exclusive; Core derives a distinct
...
pub fn ensure_single_managed_secrets_proposal_selector(
    ...
    reference_env_credential_proposal_id: Option<uuid::Uuid>,
    ...
    let selected = usize::from(connector_binding_proposal_id.is_some())
        ...
        + usize::from(reference_env_credential_proposal_id.is_some());
    if selected > 1 {
        return Err(ValidationError::new(
            "At most one managed Passwords proposal selector may be set (connector, model, publisher, or reference environment).",
        ));
    }
    Ok(())
}
...
fn ensure_delegation_setup_context(
    organization_id: uuid::Uuid,
    detail: &crate::ManagedAgentDeploymentDetail,
    request: &crate::DelegationPolicyRequestView,
) -> Result<(), ValidationError> {
...
pub fn managed_reference_env_credential_effective_mapping(
    requested_environment_names: &[String],
    request: &crate::DelegationPolicyRequestView,
) -> Result<Vec<crate::ManagedReferenceEnvCredentialEffectiveMapping>, ValidationError> {
    let requested: std::collections::BTreeSet<&str> = requested_environment_names
        .iter()
        .map(String::as_str)
        .collect();
    ...
    for entry in &request.effective_mapping {
        ...
        mapping.push(crate::ManagedReferenceEnvCredentialEffectiveMapping {
            environment_name: entry.environment_name.clone(),
            ref_uri: ref_uri.to_string(),
            vault_id: entry.vault_id,
            item_id: entry.item_id,
...
/// The decision a caller must carry out to apply a reference-environment credential
/// proposal: either the proposal is already applied (no mutation), or a single
/// proposal-bound apply call must be issued with the server-returned mapping.
#[derive(Debug, Clone)]
pub enum ReferenceEnvCredentialProposalApply {
    /// The proposal is already applied at this revision; retrying is a no-op.
    AlreadyApplied {
        applied_revision_id: uuid::Uuid,
        result_id: Option<uuid::Uuid>,
    ...
    Apply {
        proposal_id: uuid::Uuid,
        idempotency_key: uuid::Uuid,
        request: Box<crate::ApplyReferenceEnvCredentialProposalRequest>,
    },
}

pub fn reference_env_credential_proposal_applied_revision(
    reviewed: &crate::ManagedReferenceEnvCredentialProposal,
    request: &crate::ApplyReferenceEnvCredentialProposalRequest,
    applied: &crate::ManagedReferenceEnvCredentialProposal,
) -> Result<uuid::Uuid, ValidationError> {
    if applied.state != crate::ManagedReferenceEnvCredentialProposalState::Applied
        || applied.id != reviewed.id
        || applied.deployment_id != reviewed.deployment_id
        || applied.proposal_fingerprint != reviewed.proposal_fingerprint
        || applied.requirements_fingerprint != reviewed.requirements_fingerprint
    ...
    {
        return Err(ValidationError::new(
            "Core returned a reference-environment credential proposal that does not match the reviewed apply.",
        ));
    }
    applied.applied_revision_id.ok_or_else(|| {
        ValidationError::new(
            "Core returned an applied reference-environment credential proposal without an applied revision.",
        )
    })
}
...
fn reference_env_credential_proposal_already_applied(
    proposal: &crate::ManagedReferenceEnvCredentialProposal,
) -> Result<ReferenceEnvCredentialProposalApply, ValidationError> {
    let applied_revision_id = proposal.applied_revision_id.ok_or_else(|| {
        ValidationError::new(
            "The reference-environment credential proposal is applied but names no active revision.",
        )
    })?;
    Ok(ReferenceEnvCredentialProposalApply::AlreadyApplied {
        applied_revision_id,
        result_id: proposal.result_id,
    })
}

/// Decide whether a reference-environment credential proposal needs one apply mutation.
///
/// Core exposes only the deployment's latest proposal, so `proposal_id`
/// pins the apply to the proposal the caller reviewed. Fails closed: a
/// superseded proposal, a proposal bound to a different result, a stale active
...
/// mutates twice; a setup supplied on that retry must still be the one bound to
/// the proposal.
pub fn reference_env_credential_proposal_apply(
    organization_id: uuid::Uuid,
    detail: &crate::ManagedAgentDeploymentDetail,
    request: Option<&crate::DelegationPolicyRequestView>,
    proposal: &crate::ManagedReferenceEnvCredentialProposal,
    ...
    if proposal_id != proposal.id {
        return Err(ValidationError::new(
            "The managed agent's current reference-environment credential proposal is not the proposal selected for apply.",
        ));
    }
    if proposal.deployment_id != detail.deployment_id {
        return Err(ValidationError::new(
            "The reference-environment credential proposal belongs to another managed agent deployment.",
        ));
    }
    if proposal.state == crate::ManagedReferenceEnvCredentialProposalState::Superseded {
        return Err(ValidationError::new(
            "The reference-environment credential proposal has been superseded and can no longer be applied.",
        ));
    }
    ...
    let apply_request = if proposal.requires_secret_resolution_result {
        if proposal.state == crate::ManagedReferenceEnvCredentialProposalState::Applied {
            ...
            return reference_env_credential_proposal_already_applied(proposal);
        }
        let request = request.ok_or_else(|| {
            ValidationError::new(
                "The reference-environment credential proposal requires an approved Seren Passwords setup.",
            )
        })?;
        ensure_delegation_setup_context(organization_id, detail, request)?;
        ensure_setup_targets_reference_env_credential_proposal(request, proposal)?;
        ...
        let effective_mapping = managed_reference_env_credential_effective_mapping(
            &proposal.requested_environment_names,
            request,
        )?;
        crate::ApplyReferenceEnvCredentialProposalRequest {
            effective_mapping,
            secret_resolution_result_id: Some(request.result_id),
        }
    } else {
        ...
        {
            return Err(ValidationError::new(
                "The reference-environment credential proposal has an inconsistent result contract.",
            ));
        }
        if proposal.state == crate::ManagedReferenceEnvCredentialProposalState::Applied {
            return reference_env_credential_proposal_already_applied(proposal);
        }
        crate::ApplyReferenceEnvCredentialProposalRequest::default()
    };
    ...
    if proposal.expected_active_revision_id != active_revision_id {
        return Err(ValidationError::new(
            "The reference-environment credential proposal no longer targets the managed agent's active revision.",
        ));
    }

    Ok(ReferenceEnvCredentialProposalApply::Apply {
        proposal_id: proposal.id,
        idempotency_key: proposal.id,
        request: Box::new(apply_request),
    })
...
mod tests {
        ...
        for mask in 0u8..16 {
            let selectors =
                std::array::from_fn::<_, 4, _>(
                    |index| {
                        if mask & (1 << index) != 0 { one } else { None }
                    },
                );
            let result = ensure_single_managed_secrets_proposal_selector(
                selectors[0],
                selectors[1],
                selectors[2],
                selectors[3],
            );
            assert_eq!(result.is_ok(), mask.count_ones() <= 1);
        }
    }
    ...
    fn reference_env_proposal(
        deployment_id: uuid::Uuid,
        expected_active_revision_id: uuid::Uuid,
        state: &str,
    ) -> crate::ManagedReferenceEnvCredentialProposal {
        serde_json::from_value(serde_json::json!({
            "id": uuid::Uuid::new_v4(),
            "deployment_id": deployment_id,
            "expected_active_revision_id": expected_active_revision_id,
    ...
    #[test]
    fn reference_env_credential_apply_binds_server_mapping_and_is_deterministic() {
        let organization_id = uuid::Uuid::new_v4();
        let deployment_id = uuid::Uuid::new_v4();
        let revision_id = uuid::Uuid::new_v4();
        let detail = managed_detail(deployment_id, revision_id);
        ...
        let ReferenceEnvCredentialProposalApply::Apply {
            proposal_id,
            idempotency_key,
            request: apply,
    ...
    #[test]
    fn reference_env_proposal_uses_its_bound_identity_without_changing_base_setup_rules() {
        let organization_id = uuid::Uuid::new_v4();
        let deployment_id = uuid::Uuid::new_v4();
        let revision_id = uuid::Uuid::new_v4();
        let mut detail = managed_detail(deployment_id, revision_id);
        ...
        assert!(ensure_delegation_setup_applies(organization_id, &detail, &request).is_err());
        assert!(
            reference_env_credential_proposal_apply(
                organization_id,
                &detail,
    ...
    #[test]
    fn reference_env_credential_apply_rejects_a_proposal_other_than_the_selected_one() {
        let organization_id = uuid::Uuid::new_v4();
        let deployment_id = uuid::Uuid::new_v4();
        let revision_id = uuid::Uuid::new_v4();
        let detail = managed_detail(deployment_id, revision_id);
    ...
    #[test]
    fn reference_env_credential_apply_is_idempotent_when_already_applied() {
        let organization_id = uuid::Uuid::new_v4();
        let deployment_id = uuid::Uuid::new_v4();
        let revision_id = uuid::Uuid::new_v4();
        let detail = managed_detail(deployment_id, revision_id);
        let request = managed_secrets_policy_request(organization_id, deployment_id, revision_id);
        let mut proposal = reference_env_proposal(deployment_id, revision_id, "applied");
        let applied = uuid::Uuid::new_v4();
        proposal.applied_revision_id = Some(applied);
        proposal.result_id = Some(request.result_id);
        proposal.approval_request_id = Some(request.request_id);
        ...
        for setup in [Some(&request), None] {
            match reference_env_credential_proposal_apply(
                organization_id,
                &detail,
                setup,
                &proposal,
            ...
            .expect("applied proposal resolves without error")
            {
                ReferenceEnvCredentialProposalApply::AlreadyApplied {
                    applied_revision_id,
                    result_id,
                } => {
                    assert_eq!(applied_revision_id, applied);
                ...
                ReferenceEnvCredentialProposalApply::Apply { .. } => {
                    panic!("an applied proposal must not mutate again")
                }
            }
        }
    ...
    #[test]
    fn reference_env_credential_apply_supports_proposals_without_secret_resolution() {
        let organization_id = uuid::Uuid::new_v4();
        let deployment_id = uuid::Uuid::new_v4();
        let revision_id = uuid::Uuid::new_v4();
        let detail = managed_detail(deployment_id, revision_id);
        ...
        let applied_revision_id = uuid::Uuid::new_v4();
        let mut applied = proposal.clone();
        applied.state = crate::ManagedReferenceEnvCredentialProposalState::Applied;
        applied.applied_revision_id = Some(applied_revision_id);
        assert_eq!(
            reference_env_credential_proposal_applied_revision(&proposal, &request, &applied)
                .expect("matching response"),
            applied_revision_id
        );
        match reference_env_credential_proposal_apply(
    ...
    #[test]
    fn reference_env_credential_apply_fails_closed() {
        let organization_id = uuid::Uuid::new_v4();
        let deployment_id = uuid::Uuid::new_v4();
        let revision_id = uuid::Uuid::new_v4();
        let detail = managed_detail(deployment_id, revision_id);
        ...
        let superseded = reference_env_proposal(deployment_id, revision_id, "superseded");
        assert!(
            reference_env_credential_proposal_apply(
                organization_id,
                &detail,
        ...
        let mut stale =
            reference_env_proposal(deployment_id, uuid::Uuid::new_v4(), "awaiting_review");
        stale.approval_request_id = Some(request.request_id);
        assert!(
            reference_env_credential_proposal_apply(
                organization_id,
        ...
        let mut mismatched = reference_env_proposal(deployment_id, revision_id, "awaiting_review");
        mismatched.result_id = Some(uuid::Uuid::new_v4());
        mismatched.approval_request_id = Some(request.request_id);
        assert!(
            reference_env_credential_proposal_apply(
        ...
        let other = reference_env_proposal(uuid::Uuid::new_v4(), revision_id, "awaiting_review");
        assert!(
            reference_env_credential_proposal_apply(
                organization_id,
                &detail,
        ...
        let mut extra = reference_env_proposal(deployment_id, revision_id, "awaiting_review");
        extra.approval_request_id = Some(request.request_id);
        extra
            .requested_environment_names
            .push("USERNAME".to_string());
        ...
        let mut unbound = reference_env_proposal(deployment_id, revision_id, "awaiting_review");
        unbound.approval_request_id = Some(request.request_id);
        assert!(
            reference_env_credential_proposal_apply(
                organization_id,
